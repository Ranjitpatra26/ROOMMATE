...
export const LivePermissionModal: React.FC<LivePermissionModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/65 backdrop-blur-md animate-in fade-in duration-200">
      <div className="w-full max-w-md bg-[#1e2433]/85 dark:bg-[#121620]/90 backdrop-blur-2xl border border-white/20 rounded-3xl p-6 space-y-4 animate-in zoom-in-95 duration-200 text-left text-white shadow-2xl">
        {/* Header Icon & Security Badge */}
        <div className="flex items-center justify-between">
          <div className="w-12 h-12 rounded-2xl bg-vitality-coral/20 text-vitality-coral flex items-center justify-center border border-vitality-coral/30">
            <Radio className="w-6 h-6 animate-pulse" />
          </div>
          <div className="flex items-center gap-1.5 px-3 py-1 bg-trust-teal/20 text-trust-teal rounded-full font-sans text-xs font-bold border border-trust-teal/30">
        ...
        {/* Audience Preview */}
        <div className="p-3 bg-white/10 rounded-2xl border border-white/15 flex items-center justify-between font-sans text-xs">
          <span className="text-slate-300">Visible to:</span>
        ...
        <div className="p-3.5 bg-white/10 rounded-2xl border border-white/15 space-y-2.5 font-sans text-xs">
          ...
          <div className="flex items-start gap-2.5">
            <Ghost className="w-4 h-4 text-slate-300 shrink-0 mt-0.5" />
            <div>
              <span className="font-bold text-white block">
        ...
        <div className="grid grid-cols-2 gap-3 pt-2 font-sans text-xs font-bold">
          <button
            type="button"
            onClick={onCancel}
            className="py-3 bg-white/10 hover:bg-white/20 text-slate-200 hover:text-white border border-white/20 rounded-xl cursor-pointer uppercase tracking-wider transition-all"
          >
            Cancel
          </button>
