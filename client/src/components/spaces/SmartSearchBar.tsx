...
export const SmartSearchBar: React.FC<SmartSearchBarProps> = ({
  ...
  return (
    <div ref={containerRef} className={`relative pointer-events-auto ${className}`}>
      {/* Search Input Bar */}
      <div className="flex items-center gap-2.5 bg-clay/95 dark:bg-earth-container/95 backdrop-blur-2xl px-3.5 py-2.5 rounded-full border border-surface-dim dark:border-white/20 shadow-2xl transition-all focus-within:ring-2 focus-within:ring-vitality-coral/40 focus-within:border-vitality-coral">
        ...
        <input
          ...
          className="bg-transparent border-none font-sans text-xs sm:text-sm text-earth-indigo placeholder-secondary/60 dark:placeholder-slate-400 w-44 sm:w-72 md:w-84 focus:outline-none"
        />
        ...
        {query && (
          <button
            ...
            className="text-secondary hover:text-earth-indigo transition-colors p-0.5 rounded-full hover:bg-surface-low dark:hover:bg-white/10 cursor-pointer"
      ...
      {isOpen && (
        <div className="absolute bottom-full mb-2 left-0 w-[calc(100vw-3rem)] sm:w-[360px] md:w-[400px] max-w-[calc(100vw-2rem)] bg-clay/95 dark:bg-earth-container/95 backdrop-blur-2xl border border-surface-dim dark:border-white/20 rounded-3xl shadow-2xl overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-150">
          {query.trim().length >= 2 ? (
            <div>
              {/* Header */}
